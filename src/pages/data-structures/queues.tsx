...
function EnqueueDialog({ open, onOpenChange, value, onValueChange, onEnqueue, onCancel }: EnqueueDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700">
        ...
        <DialogFooter>
          ...
          <Button onClick={onEnqueue} className="bg-purple-600 hover:bg-purple-700">
            Enqueue
          </Button>
        </DialogFooter>
      </DialogContent>
  ...
  const renderQueueVisualization = () => {
    return (
      <div className="relative bg-white/10 dark:bg-slate-900/60 p-6 rounded-2xl border-2 border-transparent shadow-lg backdrop-blur-md h-full flex flex-col">
        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Queue Visualization</h3>
          ...
          <div className="flex flex-row gap-2 items-center">
            {queue.map((value, idx) => (
              <div
                key={idx}
                className={`
                  w-16 h-16 flex items-center justify-center rounded-2xl border-2 shadow-2xl cursor-pointer focus:ring-2 focus:ring-blue-400 outline-none
                  ${activeIndex === idx && animationState === "dequeueing"
                    ? "border-red-500 animate-pulse"
                    : activeIndex === idx && animationState === "peeking"
                ...
                `}
              >
                <span className="text-2xl font-extrabold drop-shadow-lg tracking-wide select-none" style={{textShadow: '0 2px 8px rgba(0,0,0,0.25)'}}>
                  {value}
                </span>
                {idx === 0 && (
