...
function InsertDialog({ open, onOpenChange, value, onValueChange, index, onIndexChange, onInsert, onCancel, maxIndex }: InsertDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700">
        <DialogHeader>
...
function RemoveDialog({ open, onOpenChange, index, onIndexChange, onRemove, onCancel, maxIndex }: RemoveDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700">
        <DialogHeader>
...
System.out.println(list.printList()); // 10 -> 15 -> 30 -> null`,
      ...
      <InsertDialog
        ...
        onValueChange={e => setNewValue(e.target.value)}
        index={index}
        onIndexChange={e => setIndex(e.target.value)}
        onInsert={handleInsert}
        onCancel={() => setIsInsertDialogOpen(false)}
        maxIndex={linkedList.length}
      />
      <RemoveDialog
