...
function PushDialog({ open, onOpenChange, value, onValueChange, onPush, onCancel }: PushDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700">
        ...
        <DialogFooter>
          ...
          <Button onClick={onPush} className="bg-purple-600 hover:bg-purple-700">
            Push
          </Button>
        </DialogFooter>
      </DialogContent>
...
export default function StacksPage() {
  ...
  const renderStackVisualization = () => {
    return (
      <div className="relative bg-white/10 dark:bg-slate-900/60 p-6 rounded-2xl border-2 border-transparent shadow-lg backdrop-blur-md h-full flex flex-col">
        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Stack Visualization</h3>
        ...
        <div className="flex-grow flex items-center justify-center">
          <div className="flex flex-col-reverse gap-2 items-center">
            ...
            {animationState === "pushing" && animationStep === 1 && newElement !== null && (
              <div
                className="w-48 h-12 flex items-center justify-center rounded-xl border-2 border-green-500 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 animate-bounce mb-4 shadow-xl"
                style={{ transform: "translateY(-20px)" }}
              >
                <span className="text-lg font-bold">{newElement}</span>
              </div>
            )}
            ...
            {stack.map((value, idx) => (
              <div
                key={idx}
                className={`
                  w-48 h-12 flex items-center justify-center rounded-xl border-2 shadow-lg relative
                  ${
                    idx === stack.length - 1
                      ? "border-cyan-400 bg-white/30 dark:bg-slate-900/40 backdrop-blur-md text-white ring-2 ring-cyan-300 z-30"
                      : "border-white/30 dark:border-slate-700 bg-white/20 dark:bg-slate-900/30 backdrop-blur-md text-white hover:scale-105 hover:shadow-2xl transition-all z-20"
                  }
                  transition-all duration-300
                  ${activeIndex === idx && animationState === "popping" ? "border-red-500 animate-pulse" : ""}
                  ${activeIndex === idx && animationState === "peeking" ? "border-blue-500 animate-pulse" : ""}
                `}
              >
                <span className="text-lg font-bold drop-shadow-lg tracking-wide select-none" style={{textShadow: '0 2px 8px rgba(0,0,0,0.25)'}}>
                  {value}
                </span>
                {idx === stack.length - 1 && (
                  <span className="absolute -right-16 text-sm text-cyan-400 font-bold">← Top</span>
                )}
              </div>
            ))}

            <div className="w-48 h-2 bg-slate-300 dark:bg-slate-600 rounded-b-xl mt-2" />
          </div>
        </div>
...
System.out.println(stack.size()); // 2`,
      ...
      <PushDialog
        ...
        onPush={handlePush}
        onCancel={() => setIsPushDialogOpen(false)}
      />
    </>
  )
}
